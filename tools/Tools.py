...
class ToolManager(DbManager):
    ...
    def initGames(self):
        ...
        try:
            ...
            sql_file = os.path.join(os.path.dirname(__file__), '../utils', 'insert_game_table.sql')
            logger.info(f"read file {sql_file}...")
            with open(sql_file, 'r', encoding='utf-8') as f:
                sql_script = f.read()

                # executescript est idéal pour exécuter plusieurs instructions (CREATE + INSERT) d'un coup
                logger.info("Run SQL script...")
                c.executescript(sql_script)
            
            # Validation des modifications
            conn.commit()
            logger.info(f"DB  is ready.")
